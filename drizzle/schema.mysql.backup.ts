...
export const users = mysqlTable("users", {
  ...
  subscriptionTier: mysqlEnum("subscriptionTier", ["none", "trial", "monthly", "yearly"]).default("none").notNull(),
  subscriptionStatus: mysqlEnum("subscriptionStatus", ["inactive", "active", "expired", "cancelled"]).default("inactive").notNull(),
  subscriptionStartDate: timestamp("subscriptionStartDate"),
  subscriptionEndDate: timestamp("subscriptionEndDate"),
  screeningsRemaining: int("screeningsRemaining").default(0),
  whitelistStatus: mysqlEnum("whitelistStatus", ["pending", "approved", "rejected"]).default("pending").notNull(),
...
/**
 * Access Requests
 * Stores user requests for access to the platform
 */
export const accessRequests = mysqlTable("access_requests", {
  id: varchar("id", { length: 64 }).primaryKey(),
  email: varchar("email", { length: 320 }).notNull(),
  requestedTier: mysqlEnum("requestedTier", ["trial", "monthly", "yearly"]).notNull(),
 ...
 * Tracks subscription changes and payments
 */
export const subscriptionHistory = mysqlTable("subscription_history", {
  id: varchar("id", { length: 64 }).primaryKey(),
  userId: varchar("userId", { length: 64 }).notNull(),
  tier: mysqlEnum("tier", ["trial", "monthly", "yearly"]).notNull(),
  action: mysqlEnum("action", ["created", "renewed", "upgraded", "downgraded", "cancelled", "expired"]).notNull(),
