...
export default function VisionScreening() {
  ...
  const [photoscreeningAssessment, setPhotoscreeningAssessment] = useState<any>(null);
  ...
  const [currentOptotype, setCurrentOptotype] = useState<string>("");
  const [recentOptotypes, setRecentOptotypes] = useState<string[]>([]);
  ...
  const startCamera = async () => {
    try {
      ...
      alert(error instanceof Error ? error.message : "Camera access is required for photoscreening.");
    }
  };
  ...
  const handlePhotoscreeningCapture = async () => {
    try {
      ...
      if (!quality.acceptable) {
        alert(`Image quality issues: ${quality.issues.join(", ")}. Please retake the photo.`);
        return;
      }
      ...
      if (eyes.length < 2) {
        alert("Could not detect both eyes. Please try again.");
        return;
      }
      
      // Analyze red reflex for both eyes
      ...
      const alignment = analyzeEyeAlignment(eyes[0], eyes[1], { x: imgData.width / 2, y: imgData.height / 2 });
      
      // Create photoscreening result
      const photoscreeningResult: PhotoscreeningResult = {
        redReflexLeft: leftEyeAnalysis.whiteReflexDetected ? "abnormal" : leftEyeAnalysis.quality === "poor" ? "unclear" : "normal",
        redReflexRight: rightEyeAnalysis.whiteReflexDetected ? "abnormal" : rightEyeAnalysis.quality === "poor" ? "unclear" : "normal",
        ...
        confidence: Math.round((leftEyeAnalysis.quality === "good" && rightEyeAnalysis.quality === "good") ? 90 : 70),
      };
      
      // Assess the photoscreening
      const assessment = assessPhotoscreening(photoscreeningResult);
      
      setPhotoscreeningResults(photoscreeningResult);
      setPhotoscreeningAssessment(assessment);
      
      // Move to visual acuity test
      setStep("visual-acuity");
      stopCamera();
      
      // Initialize visual acuity test
      if (ageGroup) {
        const optotype = generateOptotype(ageGroup === "0-2" || ageGroup === "3-5" ? "lea_symbols" : "snellen");
        setCurrentOptotype(optotype);
      }
    } catch (error) {
      console.error("Photoscreening analysis error:", error);
  ...
  const handleAcuityResponse = (canRead: boolean) => {
    ...
    // Generate next optotype
    const nextOptotype = generateOptotype(
      ageGroup === "0-2" || ageGroup === "3-5" ? "lea_symbols" : "snellen",
      [...recentOptotypes, currentOptotype]
    );
    setCurrentOptotype(nextOptotype);
    setRecentOptotypes([...recentOptotypes.slice(-2), currentOptotype]);

    // Check if we have enough responses to calculate acuity
    if (newResponses.length >= 5) {
      const correctCount = newResponses.filter(r => r).length;
      const result = calculateVisualAcuity(correctCount, newResponses.length, currentSize);
      
      if (ageGroup && ageGroup !== "") {
        const assessment = assessVisualAcuity(result.acuity, ageGroup, currentEye2);
        
        setVisualAcuityResults((prev: any) => ({
          ...prev,
          [currentEye2]: {
            acuityMeasurement: result.acuity,
            acuityDecimal: result.decimal,
            testMethod: ageGroup === "0-2" || ageGroup === "3-5" ? "lea_symbols" : "snellen",
            distanceMeters: 6,
      ...
      // Switch to other eye or finish
      if (currentEye2 === "left") {
        setCurrentEye2("right");
        setAcuityResponses([]);
        setRecentOptotypes([]);
        setCurrentSize("20/40");
      } else {
        setStep("results");
      }
    }
